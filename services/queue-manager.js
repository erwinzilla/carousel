...
class PrintQueue {
    ...
    addJob(job) {
        ...
        this.saveQueue();
        console.log(`📥 Job added to queue: ${queueJob.id}, total queue: ${this.queue.length}`);
        return queueJob;
    }

    // Ambil semua job untuk agent tertentu
    getJobsForAgent(agentId) {
        return this.queue.filter(job => job.agentId === agentId && job.status === 'pending');
    }
    ...
    async processQueueForAgent(agentId, agentSocket, pendingJobsMap) {
        ...
        console.log(`📤 Processing ${pendingJobs.length} pending jobs for ${agentId}`);
